...
import { BusinessConfigurationUpdateComponent } from '../../business-configuration/update/business-configuration-update.component';

@Component({
  standalone: true,
  selector: 'jhi-business-update',
  templateUrl: './business-update.component.html',
  imports: [SharedModule, FormsModule, ReactiveFormsModule, BusinessConfigurationUpdateComponent],
})
export class BusinessUpdateComponent implements OnInit {
  ...
  save(): void {
    ...
    if (business.id !== null) {
      business.users = this.business?.users;
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IBusiness>>): void {
    result.pipe(finalize(() => this.onSaveFinalize())).subscribe({
      next: res => {
        this.businessId = JSON.parse(JSON.stringify(res.body)).id;
        this.onSaveSuccess();
      },
