...
export class CategoryService {
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.businessService.businessData$.pipe(
      switchMap(business => {
        if (business) {
          const resourceUrlForQuery = this.applicationConfigService.getEndpointFor('api/' + business.id + '/categories');
          return this.http.get<ICategory[]>(resourceUrlForQuery, { params: options, observe: 'response' });
        } else {
          return this.businessService.query().pipe(
            switchMap(res => {
              const resourceUrlForQuery = this.applicationConfigService.getEndpointFor('api/' + res.body![0].id + '/categories');
              return this.http.get<ICategory[]>(resourceUrlForQuery, { params: options, observe: 'response' });
            }),
          );
        }
